...
def generate_wfs_huc12_intersection_url(lat, lon):
    wfs_url = (
        GS_BASE_URL
        # have to add area_type = 'HUC12' to this query now to get HUC12s in the all_areas layer
        + f"wfs?service=WFS&version=1.0.0&request=GetFeature&typeName=playground:all_areas&propertyName=(id)&outputFormat=application/json&cql_filter=INTERSECTS(the_geom, POINT({lon} {lat})) AND area_type='HUC12"
