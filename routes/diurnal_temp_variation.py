...
diurnal_temp_variation_api = Blueprint("diurnal_temp_variation_api", __name__)
diurnal_temp_variation_dim_encodings = asyncio.run(get_dim_encodings("mean_annual_dtr"))
...
def package_diurnal_temp_variation_point_data(point_data, horp):
    """Package JSON response data for diurnal_temp_variation
    ...
    if horp == "all":
        for mi, v_li in enumerate(point_data):  # (nested list with model at dim 0)
            ...
            model = diurnal_temp_variation_dim_encodings["model"][mi]
            point_pkg[model] = {}
            ...
            for value in v_li:
                if year in years:
                    point_pkg[model][years[year_index]] = {"madtr": round(value)}
                    year_index += 1
                year += 1
    else:
        if horp in ["historical", "hp"]:
            ...
            point_pkg["historical"]["madtrmin"] = round(historical_min)
            point_pkg["historical"]["madtrmean"] = round(historical_mean)
            point_pkg["historical"]["madtrmax"] = round(historical_max)
        ...
        if horp in ["projected", "hp"]:
            ...
            point_pkg["projected"] = {}
            point_pkg["projected"]["madtrmin"] = round(projected_min)
            point_pkg["projected"]["madtrmean"] = round(projected_mean)
            point_pkg["projected"]["madtrmax"] = round(projected_max)

    return point_pkg


async def fetch_diurnal_temp_variation_point_data(x, y, horp, start_year, end_year):
    ...
    point_data_list = []
    if horp == "all":
        request_str = generate_wcs_getcov_str(x, y, "mean_annual_dtr")
    ...
    if horp in ["historical", "hp"]:
        ...
        for summary_stat in range(0, 3):
            request_str = generate_wcps_request_str(
                x, y, "mean_annual_dtr", "0:0", timestring, summary_stat
            )
    ...
    if horp in ["projected", "hp"]:
        ...
        for summary_stat in range(0, 3):
            request_str = generate_wcps_request_str(
                x, y, "mean_annual_dtr", "1:2", timestring, summary_stat
            )
            point_data_list.append(
                await fetch_data([generate_wcs_query_url(request_str)])
            )
...
def create_csv(data_pkg, lat=None, lon=None):
    ...
    metadata = "# madtr is the mean annual diurnal temperature variation (°C)\n"
    filename = (
        "Mean Annual Diurnal Temperature Variation"
        + " for "
        + lat
        + ", "
        + lon
...
# CP: consider if routing synonyms (top-level and mmm-nested) make sense for this endpoint. another alternative could be /precip_inidcators/diurnal_temp_variation...
@routes.route("/diurnal_temp_variation/")
@routes.route("/diurnal_temp_variation/abstract/")
@routes.route("/mmm/diurnal_temp_variation/")
@routes.route("/mmm/diurnal_temp_variation/abstract/")
def diurnal_temp_variation_about():
    return render_template("/diurnal_temp_variation/abstract.html")


@routes.route("/mmm/diurnal_temp_variation/point")
@routes.route("/diurnal_temp_variation/point")
def diurnal_temp_variation_about_point():
    return render_template("/diurnal_temp_variation/point.html")


@routes.route("/diurnal_temp_variation/<horp>/point/<lat>/<lon>")
@routes.route(
    "/diurnal_temp_variation/<horp>/point/<lat>/<lon>/<start_year>/<end_year>"
)
@routes.route("/mmm/diurnal_temp_variation/<horp>/point/<lat>/<lon>")
@routes.route(
    "/mmm/diurnal_temp_variation/<horp>/point/<lat>/<lon>/<start_year>/<end_year>"
)
def run_fetch_diurnal_temp_variation_point_data(
    lat, lon, horp, start_year=None, end_year=None
):
    """Mean Annual Diurnal Temperature Variation data endpoint. Fetch point data for
    ...
    Returns:
        JSON-like dict of requested Mean Annual Diurnal Temperature Variation data

    Notes:
        example request: http://localhost:5000/mmm/diurnal_temp_variation/all/65/-147
    """
    ...
    try:
        point_data_list = asyncio.run(
            fetch_diurnal_temp_variation_point_data(x, y, horp, start_year, end_year)
        )
    ...
    point_pkg = package_diurnal_temp_variation_point_data(point_data_list, horp)

    if request.args.get("format") == "csv":
        point_pkg = nullify_and_prune(point_pkg, "diurnal_temp_variation")
        if point_pkg in [{}, None, 0]:
            return render_template("404/no_data.html"), 404
        return create_csv(point_pkg, lat=lat, lon=lon)

    return postprocess(point_pkg, "diurnal_temp_variation")


@routes.route("/eds/diurnal_temp_variation/point/<lat>/<lon>")
def get_diurnal_temp_variation_plate(lat, lon):
    """
    Endpoint for requesting all data required for Mean Annual Diurnal Temperature Variation in the Arctic-EDS client.
    Args:
        lat (float): latitude
        lon (float): longitude
    Notes:
        example request: http://localhost:5000/eds/diurnal_temp_variation/point/65.0628/-146.1627
    """
    madtr_plate = {}

    results = run_fetch_diurnal_temp_variation_point_data(lat, lon, "historical")
    madtr_plate["historical"] = results["historical"]

    results = run_fetch_diurnal_temp_variation_point_data(
        lat, lon, "projected", start_year="2010", end_year="2039"
    )
    madtr_plate["2010-2039"] = results["projected"]

    results = run_fetch_diurnal_temp_variation_point_data(
        lat, lon, "projected", start_year="2040", end_year="2069"
    )
    madtr_plate["2040-2069"] = results["projected"]

    results = run_fetch_diurnal_temp_variation_point_data(
        lat, lon, "projected", start_year="2070", end_year="2099"
    )
    madtr_plate["2070-2099"] = results["projected"]

    return jsonify(madtr_plate)
