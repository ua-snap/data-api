...
for coverage_id in fire_weather_coverage_ids:
    ...
    var_coverage_metadata[list(coverage_metadata["metadata"]["bands"].keys())[0]] = {
                                    ...
                                    "model_encoding": ast.literal_eval(coverage_metadata["metadata"]["axes"]["model"]["encoding"]),
...
def validate_years(start_year, end_year, var):
    ...
    time_units, time_min, time_max = var_coverage_metadata[var]["time_units"], var_coverage_metadata[var]["time_min"], var_coverage_metadata[var]["time_max"]
...
def run_fetch_fire_weather_point_data(lat, lon, start_year=None, end_year=None):
    ...
    if requested_vars:
        ...
        coverage_ids = [var_coverage_metadata[var]["coverage_id"] for var in requested_vars]
    else:
    ...
    for var in requested_vars:
        ...
        # otherwise, filter by the validated times
            ...
            print(f"time filtering on {var_coverage_metadata[var]['coverage_id']} from {times[0]} to {times[1]}")
