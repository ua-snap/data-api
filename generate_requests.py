...
def generate_wcs_getcov_str(
    x, y, cov_id, var_coord=None, var_slice=None, encoding="json"
):
    """Generate a WCS GetCoverage request for fetching a
    subset of a coverage over X and Y axes.
    ...
    Args:
        ...
        var_slice (tuple): two-tuple of a Rasdaman coverage axis name (e.g., `year`) and the axis value desired to slice the data
    ...
    if var_slice is not None:
        var_axis, var_string = var_slice
        var_slice_str = f"&SUBSET={var_axis}({var_string})"
    else:
        var_slice_str = ""
    wcs_getcov_str = (
        f"GetCoverage&COVERAGEID={cov_id}"
        f"&SUBSET=X({x})&SUBSET=Y({y}){var_subset_str}{var_slice_str}"
        f"&FORMAT=application/{encoding}"
    )
    return wcs_getcov_str
