...
def _mock_landslide_row(place_id, place_name):
...
def _mock_place_data(community_id, name):
    return {
        "id": community_id,
        "name": name,
        "alt_name": "",
...
def test_landslide_ak91(client, monkeypatch):
    """
    Tests the /landslide/AK91 endpoint to ensure the output
    contains all required keys for Craig, AK.
    """
    monkeypatch.setattr(
        "routes.landslide.get_landslide_db_row",
        lambda place_name: _mock_landslide_row("AK91", "Craig"),
    )
    monkeypatch.setattr(
        "routes.landslide.get_place_data",
        lambda community_id: _mock_place_data("AK91", "Craig"),
    )
...
def test_landslide_ak182(client, monkeypatch):
    """
    ...
    monkeypatch.setattr(
        "routes.landslide.get_landslide_db_row",
        lambda place_name: _mock_landslide_row("AK182", "Kasaan"),
    )
    monkeypatch.setattr(
        "routes.landslide.get_place_data",
        lambda community_id: _mock_place_data("AK182", "Kasaan"),
    )
