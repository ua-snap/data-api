...
def summarize_within_poly_sum(ds, poly, dim_encodings, varname="Gray", roundkey="Gray"):
    """Summarize a single Data Variable of a xarray.DataSet within a polygon.
    Return the results as a nested dict. Identical to summarize_within_poly() but uses sum operator.
    ...
    results = np.nansum(data_arr, axis=(1, 2)).astype(float)
