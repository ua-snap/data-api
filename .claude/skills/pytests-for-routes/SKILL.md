...
1. Take a bounded, deterministic subset of the response that still exercises real structure. For a flat or shallow dict, the first 5 top-level keys is enough (`{k: actual_data[k] for k in list(actual_data)[:5]}`), or the first 5 items of a list. **For a deeply-nested dict (model/scenario/date-style nesting, 3+ levels), slicing only the top level is usually not enough** — the confirmed case in this repo, `cmip6_downscaled`'s point tests, nests `model -> scenario -> date -> var` with a full multi-decade daily time series per model/scenario pair, so keeping even just 5 top-level model keys still produced a ~40MB fixture (well over the cap) until the slice was bounded at every level (5 models x 2 scenarios x 5 dates each, via a small recursive/nested-loop helper — see `tests/routes/cmip6_downscaled/point/test_cmip6_downscaled_point.py`'s `_bounded_subset()` for the pattern to copy). If a route's response nests more than two levels deep, assume you'll need to bound each level, not just the outermost one.
2. **Verify the actual serialized size of the subset you're about to save** (`len(json.dumps(actual_subset))`, or check the file size after writing it) — don't just trust that "first N keys" was enough. If it's still anywhere near 25MB, bound a deeper level (fewer scenarios/dates/items per nested key) and recheck, rather than shipping an oversized fixture.
3. Save that subset as the fixture, named with a `_subset` suffix (e.g. `<route>_point_<location-slug>_subset.json`) so it's obvious at a glance that it isn't the full response.
4. In the test, take the *same* slice from the live response (reuse one small helper function per test file rather than repeating the slicing logic inline in every test — see `_bounded_subset()` above) and assert that slice equals the saved subset using `assert_json_allclose` (see above) — do not compare the full live response against the subset fixture (they won't match). Also assert something cheap about the overall shape of the full response (e.g. `isinstance(actual_data, dict)` and it's non-empty) so a gross regression (empty response, wrong type) is still caught even though the full payload isn't diffed.
5. Add a one-line comment in the test explaining why only a subset is compared (payload exceeds the 25MB fixture cap).
